-- 025: variant-aware atomic checkout
...
declare v_user uuid:=auth.uid(); v_product public.products%rowtype; v_variant public.product_variants%rowtype; v_offer public.offers%rowtype; v_coupon public.coupons%rowtype;
  v_list numeric(10,2); v_price numeric(10,2); v_offer_discount numeric(10,2):=0; v_coupon_discount numeric(10,2):=0; v_total numeric(10,2);
  v_expiry_hours int:=72; v_expiry timestamptz; v_order uuid; v_existing public.orders%rowtype; v_code text:=upper(btrim(coalesce(p_coupon_code,'')));
begin
