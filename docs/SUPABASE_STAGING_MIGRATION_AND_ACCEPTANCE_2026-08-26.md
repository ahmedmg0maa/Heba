...
> تحديث مصدر محلي في 2026-08-28: تطور التسلسل المحلي بعد هذا الدليل التاريخي إلى `000–075` (76 ملفًا). بقيت Production موثقة عند `000–043` فقط، ولم تُطبق `044–075` على أي مزود في هذه الجولة؛ يجب تشغيلها بالترتيب داخل Staging المستقل بعد Recovery Point وschema fingerprint معتمدين. تضيف 074 أرشفة تسليم محمي بفصل صريح عن تنظيف Storage، وتجعل 075 revision/mutation/audit لأقسام CMS معاملة واحدة؛ كلها ما زالت `unverified` حيًا.
