...
|---|---|---|
| Sequential source migrations 000–051 | `PASS-local` | `pnpm audit:db` |
...
Production remains untouched and 043 must not be reapplied. Migrations 044–047 remain unapplied in this execution. Migrations 048–051 are additive development-first Contact, Testimonials, Press and Resource Hub contracts authored locally after 047 and are also unapplied; any provider application must preserve the full 044→051 order and requires the ordered Staging gate. Later local migrations do not authorize provider writes.
