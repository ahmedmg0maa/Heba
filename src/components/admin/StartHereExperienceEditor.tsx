...
import { START_HERE_PATHS, type StartHereContent } from '@/lib/start-here/content'

const input = 'min-h-10 w-full rounded-xl border border-line bg-surface-raised px-3 py-2 text-sm text-ink'
const pathLabels = { session: 'الجلسة', course: 'الدورة', book: 'الكتاب' }
...
export function StartHereExperienceEditor({ content }: { content: StartHereContent }) {
  ...
  return <form className="space-y-6" onSubmit={async (event) => { event.preventDefault(); setBusy(true); const result = await saveStartHereExperience(new FormData(event.currentTarget)); setMessage(result.ok ? 'حُفظت رحلة البداية وأصبحت الصفحة العامة تستخدمها.' : result.error); setBusy(false) }}>
    ...
    <fieldset className="space-y-3 rounded-xl border border-line p-4"><legend className="px-2 font-bold text-deep-teal">الاختبار الإرشادي</legend><div className="grid gap-3 md:grid-cols-2"><Field label="السطر التعريفي" name="quiz_eyebrow" value={content.quiz.eyebrow} /><Field label="العنوان" name="quiz_heading" value={content.quiz.heading} /><div className="md:col-span-2"><Field label="الوصف" name="quiz_lead" value={content.quiz.lead} area /></div></div>{content.quiz.questions.map((question, index) => <div key={index} className="grid gap-3 rounded-xl bg-ivory/55 p-3 md:grid-cols-3"><div className="md:col-span-3"><Field label={`السؤال ${index + 1}`} name={`question_${index}_title`} value={question.title} /></div>{START_HERE_PATHS.map((path) => <Field key={path} label={`إجابة ${pathLabels[path]}`} name={`question_${index}_${path}`} value={question.options[path]} />)}</div>)}</fieldset>
    <fieldset className="grid gap-3 rounded-xl border border-line p-4"><legend className="px-2 font-bold text-deep-teal">نتائج الاختبار</legend>{START_HERE_PATHS.map((path) => { const result = content.quiz.results[path]; return <div key={path} className="grid gap-3 rounded-xl bg-ivory/55 p-3 md:grid-cols-2"><h4 className="font-bold text-deep-teal md:col-span-2">مسار {pathLabels[path]}</h4><Field label="العنوان" name={`result_${path}_title`} value={result.title} /><Field label="نص الزر" name={`result_${path}_cta`} value={result.cta} /><div className="md:col-span-2"><Field label="التوضيح" name={`result_${path}_text`} value={result.text} area /></div><Field label="الرابط الداخلي" name={`result_${path}_href`} value={result.href} link /></div> })}</fieldset>
    ...
    <div className="flex flex-wrap items-center gap-3"><Button type="submit" disabled={busy}>{busy ? 'جارٍ الحفظ…' : 'حفظ ونشر رحلة البداية'}</Button>{message && <p role="status" className="text-sm font-semibold text-deep-teal">{message}</p>}</div>
  </form>
}
