    ...
    <div className="mb-8 flex flex-wrap items-center gap-3">
      <div ref={boxRef} className="relative min-w-64 flex-1">
        ...
        <input
          ...
          className="w-full rounded-full border border-line bg-soft-white py-2.5 ps-11 pe-4 text-sm text-ink shadow-card transition-colors placeholder:text-taupe/60 focus:border-deep-teal focus:outline-2 focus:outline-deep-teal/20"
        />
        {open && q.trim().length >= 2 && (
          <div className="absolute inset-x-0 top-full z-30 mt-2 overflow-hidden rounded-2xl border border-line bg-soft-white shadow-card-hover">
            {busy ? (
      ...
      <div className="flex gap-2">
        {[
          ...
          <Link
            key={l.href}
            href={l.href}
            className="hidden rounded-full border border-line bg-soft-white px-4 py-2 text-sm font-semibold text-deep-teal shadow-card transition-all hover:-translate-y-0.5 hover:border-antique-gold sm:block"
          >
            {l.label}
          </Link>
        ))}
