...
export function LessonForm({ moduleId, courseId }: { moduleId: string; courseId: string }) {
  ...
  return (
    <form
      ...
      className="grid gap-3 rounded-xl bg-ivory/60 p-4 sm:grid-cols-[2fr_1fr_2fr_auto_auto]"
    >
      <FormField label="عنوان الدرس" name="title" required />
      <FormField label="الدقائق" name="minutes" type="number" min={1} dir="ltr" />
      <FormField label="مسار الفيديو (اختياري)" name="video_path" dir="ltr" hint="داخل bucket ‏course-videos" />
      ...
      {error && <p className="text-xs text-burgundy sm:col-span-5">{error}</p>}
    </form>
  )
}
