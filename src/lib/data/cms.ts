...
export async function getContentReadiness(): Promise<ContentReadinessItem[]> {
  if (!hasEnv()) {
    return [
      ...
      { id: 'database-change', title: 'ترحيلات التشغيل والحوكمة', detail: '044–058 مصدرية محلية فقط ولم تُقبل على Staging؛ 043 وحدها هي آخر حالة Production موثقة.', status: 'blocked', href: '/admin/system' },
  ...
  try {
    ...
    return [
      ...
      { id: 'database-change', title: 'ترحيلات التشغيل والحوكمة', detail: '044–058 محلية فقط؛ يلزم Recovery ثم تطبيقها بالترتيب على Staging قبل أي إطلاق.', status: 'blocked', href: '/admin/system' },
