...
import { clearPreviewAdminSession, establishPreviewAdminSession, isPreviewAdminConfigured, verifyPreviewAdminPassword } from '@/lib/preview/admin-session'
...
export async function loginPreviewAdmin(_state: PreviewAdminLoginState, formData: FormData): Promise<PreviewAdminLoginState> {
  ...
  if (!await verifyPreviewAdminPassword(password)) {
    await new Promise((resolve) => setTimeout(resolve, 700))
    return { error: 'تعذّر الدخول. راجعي كلمة مرور المعاينة.' }
  }
