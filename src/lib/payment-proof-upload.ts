...
export async function uploadPaymentProofDirect(orderId: string, method: Method, file: File): Promise<Result> {
  ...
  const started = await beginPaymentProofUpload({ orderId, method, name: file.name, type: file.type, size: file.size, requestId })
  ...
  if (upload.error) {
    await finalizePaymentProofUpload({
      orderId, method, path: started.data.path, intentId: started.data.intentId,
      type: file.type, size: file.size,
    })
    return { ok: false, error: 'تعذّر رفع الإيصال بأمان. حاولي مرة أخرى.' }
  }
  return finalizePaymentProofUpload({
    orderId, method, path: started.data.path, intentId: started.data.intentId,
    type: file.type, size: file.size,
  })
}
